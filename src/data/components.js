...
export const COMPONENTS = [
  ...
  { id: 'enrollment', name: 'Device Enrollment', category: 'devices', icon: 'LogIn',
    ...
    prereqs: [],
    description: 'Enrola endpoints Windows, iOS/iPadOS, macOS, Android Enterprise y Linux en Intune. Soporta Automated Device Enrollment (Apple), Android Enterprise, hand-off desde Windows Autopilot, paquetes de bulk provisioning y BYOD user-driven.' },
  { id: 'config-profiles', name: 'Configuration Profiles', category: 'devices', icon: 'Settings',
    ...
    prereqs: ['enrollment'],
    description: 'Pushea settings de dispositivo vía Settings Catalog (10K+ settings Windows), ADMX importado, templates por plataforma y OMA-URI custom. Reemplaza GPOs legacy para endpoints cloud-managed.' },
  { id: 'compliance-policies', name: 'Compliance Policies', category: 'devices', icon: 'BadgeCheck',
    ...
    prereqs: ['enrollment'],
    description: 'Define reglas de salud del dispositivo (versión de OS, encryption, jailbreak, risk level de MDE). Los dispositivos non-compliant son bloqueados por Entra Conditional Access. Soporta acciones de non-compliance (notificar, retire con grace period).' },
  { id: 'scripts', name: 'Scripts & Remediations', category: 'devices', icon: 'Terminal',
    sublabel: 'PowerShell · Bash · Custom Detection/Remediation',
    prereqs: ['enrollment'],
    description: 'Ejecuta platform scripts (PowerShell en Windows, shell en macOS/Linux) y paquetes de Remediations proactivos (pares de detection + remediation con reporting). Remediations requiere licencia elegible para Endpoint Analytics.' },
  { id: 'filters', name: 'Assignment Filters & Scope Tags', category: 'devices', icon: 'Filter',
    sublabel: 'Device · User · OS · Ownership · RBAC scoping',
    prereqs: ['enrollment'],
    description: 'Los Filters acotan asignaciones de política en runtime por propiedades del dispositivo (modelo, OS, ownership). Los Scope Tags restringen la visibilidad del admin para RBAC. Críticos en despliegues multi-tenant o multi-BU.' },
  { id: 'co-management', name: 'Co-management (MECM)', category: 'devices', icon: 'GitMerge',
    ...
    prereqs: ['enrollment'],
    description: 'Migra workloads desde Configuration Manager (MECM/SCCM) a Intune un slider a la vez: compliance, device config, Endpoint Protection, Office Click-to-Run, Windows Update, client apps. Tenant Attach expone los devices de MECM en el portal de Intune.' },

  // ── App Management (MAM) ─────────────────────────────────────────────────
  { id: 'app-deployment', name: 'App Deployment', category: 'apps', icon: 'PackagePlus',
    sublabel: 'Win32 · MSIX · iOS/Android Store · LOB · Web links',
    prereqs: ['enrollment'],
    description: 'Despliega apps Win32 (.intunewin), MSIX, Microsoft Store apps, apps iOS/Android del store vía VPP/MGP, paquetes LOB y shortcuts de web-link. Intents Required / Available / Uninstall con soporte de dependencias y supersedence.' },
  { id: 'app-protection', name: 'App Protection Policies', category: 'apps', icon: 'ShieldHalf',
    ...
    prereqs: [],
    description: 'MAM tipo contenedor para apps Microsoft 365 manejadas en dispositivos personales (BYOD). Cifra los datos de la app, controla copy/paste entre contextos work y personal, requiere PIN y soporta selective wipe — sin enrolar el dispositivo.' },
  { id: 'app-config', name: 'App Configuration Policies', category: 'apps', icon: 'SlidersHorizontal',
    ...
    description: 'Pushea configuración por app (URLs de servidor pre-pobladas, hints de cuenta, feature toggles, parámetros per-app VPN) a apps manejadas en dispositivos enrolados o MAM-only. Reduce fricción de setup para el usuario final.' },
  { id: 'enterprise-app-mgmt', name: 'Enterprise App Catalog', category: 'apps', icon: 'Store',
    ...
    prereqs: ['app-deployment'],
    description: 'Catálogo curado por Microsoft de apps Windows pre-empaquetadas (Chrome, Firefox, Notepad++, 7-Zip, Adobe Reader, etc.) con soporte de actualización automática. Elimina el paso de empaquetado .intunewin. Requiere Intune Suite o el add-on Enterprise App Management.' },
  { id: 'managed-google-play', name: 'Managed Google Play', category: 'apps', icon: 'PlayCircle',
    ...
    prereqs: ['enrollment'],
    description: 'App store empresarial de Google integrada con Intune. Requerido para escenarios Android Enterprise: Work Profile (BYOD), Fully Managed (corporativo), Dedicated (kiosk/COSU). Pre-aprueba apps centralmente; los usuarios instalan desde un Play store manejado.' },
  ...
  { id: 'antivirus', name: 'Antivirus Policies', category: 'security', icon: 'Bug',
    ...
    description: 'Configura Microsoft Defender Antivirus (real-time protection, scan schedules, exclusiones, Tamper Protection, network protection). También soporta configuraciones de Defender para macOS/Linux.' },
  { id: 'disk-encryption', name: 'Disk Encryption', category: 'security', icon: 'Lock',
    sublabel: 'BitLocker · FileVault · Recovery Keys',
    prereqs: ['config-profiles'],
    description: 'Aplica BitLocker (Windows) y FileVault (macOS) con habilitación silenciosa, escrow de las recovery keys hacia Entra ID y recuperación self-service vía Company Portal.' },
  { id: 'firewall', name: 'Firewall & Rules', category: 'security', icon: 'Flame',
    sublabel: 'Windows Defender Firewall · Inbound/Outbound rules',
    prereqs: ['config-profiles'],
    description: 'Gestiona los profiles del Windows Defender Firewall (Domain/Private/Public) y reglas inbound/outbound desde la nube — incluyendo settings groups reusables para rangos IP y puertos.' },
  { id: 'edr', name: 'Endpoint Detection & Response', category: 'security', icon: 'Radar',
    ...
    description: 'Despliega el sensor de Defender for Endpoint a escala vía Intune. Pushea el onboarding blob, configura EDR in Block Mode y devuelve el risk score del dispositivo hacia Intune compliance.' },
  { id: 'asr', name: 'Attack Surface Reduction', category: 'security', icon: 'ShieldOff',
    ...
    prereqs: ['antivirus'],
    description: 'Pushea reglas ASR (bloqueo de credential stealing, Office child processes, JavaScript downloads, etc.), XML de Exploit Protection, Controlled Folder Access (anti-ransomware) y políticas de Device Control USB.' },
  { id: 'security-baselines', name: 'Security Baselines', category: 'security', icon: 'BookCheck',
    sublabel: 'Windows · MDE · Edge · M365 Apps',
    prereqs: ['config-profiles'],
    description: 'Baselines pre-configurados por Microsoft para Windows, Defender for Endpoint, Microsoft Edge y M365 Apps. Reduce el esfuerzo de hardening de semanas de diseño de GPO a una sola asignación.' },
  { id: 'account-protection', name: 'Account Protection', category: 'security', icon: 'KeyRound',
    ...
    prereqs: ['config-profiles'],
    description: 'Gestiona cuentas de administrador local (Windows LAPS — passwords respaldados en Entra ID), Windows Hello for Business (PIN/biometría) y membresía local de usuarios/grupos.' },
  ...
  { id: 'autopilot', name: 'Windows Autopilot', category: 'provision', icon: 'Rocket',
    ...
    prereqs: ['enrollment'],
    description: 'Provisioning zero-touch para Windows. Los dispositivos llegan desde el OEM con el hardware hash registrado; el primer boot dispara el enrollment completo en Intune, hace Entra Join, aplica perfiles y deja al usuario en un desktop fully-managed. Incluye la experiencia moderna Device Preparation.' },
  { id: 'esp', name: 'Enrollment Status Page', category: 'provision', icon: 'LoaderCircle',
    ...
    prereqs: ['autopilot'],
    description: 'Out-of-box experience que bloquea el desktop hasta que las apps, scripts y perfiles trackeados instalen. Reduce tickets de soporte tipo "logueado pero sin configurar". Timeouts y failure behavior configurables.' },
  { id: 'update-rings', name: 'Update Rings', category: 'provision', icon: 'CircleDot',
    sublabel: 'Pilot · Broad · Critical · Deferrals · Active hours',
    prereqs: ['enrollment'],
    description: 'Rings de Windows Update for Business. Modelo clásico pilot → broad → critical con deferral windows, active hours, deadlines, grace periods de restart y controles de pause — todo enforced vía Windows Update for Business.' },
  { id: 'feature-updates', name: 'Feature & Quality Updates', category: 'provision', icon: 'TrendingUp',
    ...
    description: 'Apunta a una versión específica de Windows feature update (ej. 23H2) y usa rollout waves escalonadas. Los profiles de Quality Updates controlan los cumulative updates mensuales con schedules. Respeta los safeguard holds de Microsoft.' },
  { id: 'driver-updates', name: 'Driver & Firmware Updates', category: 'provision', icon: 'Cpu',
    ...
    prereqs: ['update-rings'],
    description: 'Gestiona la entrega de drivers OEM y firmware vía Windows Update for Business. Revisa drivers pendientes, aprueba, pausa, o configura políticas de aprobación automática por grupo de dispositivos.' },
  ...
  { id: 'remote-help', name: 'Remote Help', category: 'suite', icon: 'Headphones',
    ...
    prereqs: ['enrollment'],
    description: 'Asistencia remota cloud-based integrada con Entra ID y RBAC de Intune. Soporta sesiones view-only y full-control, comandos elevados con UAC, chat in-session y full audit. Reemplaza RDP / Quick Assist en escenarios manejados.' },
  { id: 'epm', name: 'Endpoint Privilege Management', category: 'suite', icon: 'UserCog',
    ...
    prereqs: ['enrollment'],
    description: 'Mantiene a los usuarios como Standard y eleva únicamente procesos aprobados específicos (por hash, certificate, file path o publisher) con reporting. Reemplaza el "todos son admin local" con elevación granular y auditable por proceso.' },
  { id: 'tunnel', name: 'Microsoft Tunnel', category: 'suite', icon: 'Network',
    ...
    description: 'Gateway VPN liviano que entrega per-app VPN para iOS y Android. Soporta autenticación basada en certificado vía Cloud PKI, split tunneling e integración con Entra Conditional Access para requisitos de compliant-device.' },
  { id: 'cloud-pki', name: 'Microsoft Cloud PKI', category: 'suite', icon: 'FileKey',
    sublabel: 'Issuing CAs · SCEP · Auto-renewal · No on-prem NDES',
    prereqs: [],
    description: 'PKI cloud fully-managed para Intune. Emite certificados SCEP a dispositivos manejados para Wi-Fi, VPN, S/MIME y Tunnel sin requerir ADCS, NDES ni connectors on-prem. Soporta hasta 6 CAs en total por tenant (Root + Issuing + BYOCA combinados — ej. 1 root + 5 issuing, o 3 roots × 1 issuing c/u, etc).' },
  { id: 'endpoint-analytics', name: 'Endpoint Analytics (Advanced)', category: 'suite', icon: 'Activity',
    ...
    prereqs: ['enrollment'],
    description: 'Insights de productividad basados en telemetría para la flota manejada: boot times, app reliability, battery health, anomaly detection. Advanced Endpoint Analytics (Suite) agrega device timeline, root-cause de anomalías y retención extendida.' },
  { id: 'copilot-intune', name: 'Microsoft Copilot in Intune', category: 'suite', icon: 'Sparkles',
    ...
    prereqs: [],
    description: 'Asistente de AI embebido en el Intune admin center. Resume políticas, responde preguntas en lenguaje natural sobre dispositivos ("muéstrame dispositivos Windows 11 non-compliant de Marketing"), explica el impacto de un setting y genera queries.' },

  // ── Entra Ecosystem ──────────────────────────────────────────────────────
  { id: 'entra-id', name: 'Entra ID', category: 'entra', icon: 'Users',
    sublabel: 'Identity · Groups · Join · Hybrid Join · Registered',
    prereqs: [],
    description: 'Proveedor de identidad cloud. Los dispositivos hacen join a Entra ID directamente (cloud-only), vía Hybrid Join (con AD on-prem), o se registran (BYOD). La membresía de grupo dirige las asignaciones de Intune; los grupos dinámicos reaccionan a atributos en tiempo real.' },
  { id: 'entra-ca', name: 'Conditional Access', category: 'entra', icon: 'ShieldCheck',
    ...
    description: 'Motor de políticas que aplica los requisitos "compliant device" y "approved client app" antes de otorgar acceso a Microsoft 365, Azure y apps SAML/OIDC. Consume las señales de compliance de Intune y el estado de App Protection.' },
  { id: 'entra-private-access', name: 'Entra Private Access', category: 'entra', icon: 'Globe',
    ...
    prereqs: ['entra-id'],
    description: 'Reemplazo identity-centric ZTNA / SSE de Microsoft para las VPN legacy. Acceso per-app e identity-aware a recursos privados vía cliente Global Secure Access. Funciona en paralelo con la postura de compliance manejada por Intune.' },
  ...
  { id: 'defender-endpoint', name: 'Microsoft Defender for Endpoint', category: 'defender', icon: 'Shield',
    ...
    prereqs: [],
    description: 'EDR empresarial de Microsoft para Windows, macOS, Linux, iOS y Android. Entrega Threat & Vulnerability Management (TVM), live response, detecciones behaviorales y un risk score por dispositivo que Intune compliance puede consumir.' },
  { id: 'defender-xdr', name: 'Microsoft Defender XDR', category: 'defender', icon: 'ScanEye',
    ...
    description: 'Portal XDR que unifica alertas de MDE, MDI, MDO, MDCA y Entra ID Protection en incidentes únicos. El advanced hunting (KQL) cubre DeviceInfo, DeviceProcessEvents, DeviceLogonEvents, etc. — incluyendo telemetría de dispositivos manejados por Intune.' },
  { id: 'security-copilot', name: 'Microsoft Security Copilot', category: 'defender', icon: 'BotMessageSquare',
    ...
    prereqs: [],
    description: 'Asistente de AI para el SOC. Experiencia embebida en Defender XDR + portal standalone. Hospeda agentes autónomos incluyendo el Conditional Access Optimization Agent, que analiza políticas de CA en busca de coverage gaps sobre dispositivos Intune-compliant.' }
]
...
COMPONENTS.forEach(c => {
  ...
  const SVG_ICONS = new Set([
    ...
    // app-deployment removed — Aplicaciones.svg 9-tile grid disappears visually on the very light indigo 'apps' cat.bg circle; lucide PackagePlus is crisp.
    'entra-id','filters',
